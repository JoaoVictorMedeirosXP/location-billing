...
class RentalContractService:
  ...
  def __init__(self, firestore_repo: FirestoreRepository, big_query_repo: BigQueryRepository):
  ...
  def get_latest_contracts_by_cnpj(self, contracts):
    ...
    for contract in contracts:
        cnpj = contract['cnpj']
        if cnpj not in latest_by_cnpj or contract['contractDate'] > latest_by_cnpj[cnpj]['contractDate']:
  ...
  def set_contract_with_bills(self, contract):
    ...
    return RentalContract(
      ...
      units = [Bill(**row.to_dict()) for _, row in bills[bills['conta_contrato'].isin(units)].iterrows()],
      rental_units = [Bill(**row.to_dict()) for _, row in bills[bills['conta_contrato'].isin(rental_units)].iterrows() ]
  )
